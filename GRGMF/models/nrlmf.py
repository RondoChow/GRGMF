...
class NRLMF:
    def __init__(self, cfix=5, K1=5, K2=5, num_factors=10, theta=1.0, lambda_d=0.625, lambda_t=0.625, alpha=0.1,
                 beta=0.1, max_iter=100):
    ...
    def construct_neighborhood(self, drugMat, targetMat):
        ...
        if self.K1 > 0:# K1 is the size of neighbors
            ...
            for d, t in test_data:
                if d in self.train_drugs:
                    if t in self.train_targets:
                        ...
                        val = np.sum(self.U[d, :]*np.dot(TS[t, jj], self.V[tinx[jj], :]))/(np.sum(TS[t, jj])+eps)
                else:
                    if t in self.train_targets:
                        ii = np.argsort(DS[d, :])[::-1][:self.K2]
                        val = np.sum(np.dot(DS[d, ii], self.U[dinx[ii], :])*self.V[t, :])/(np.sum(DS[d, ii])+eps)
                    else:
                        ii = np.argsort(DS[d, :])[::-1][:self.K2]
                        jj = np.argsort(TS[t, :])[::-1][:self.K2]
                        v1 = DS[d, ii].dot(self.U[dinx[ii], :])/(np.sum(DS[d, ii])+eps)
                        v2 = TS[t, jj].dot(self.V[tinx[jj], :])/(np.sum(TS[t, jj]) + eps)
                        val = np.sum(v1*v2)
                # It's easy to cause a overflow
                # scores.append(np.exp(val)/(1+np.exp(val)))
                scores.append(1. / (1. + np.exp(-val)))
    ...
    def __str__(self):
        return "Model: NRLMF, c:%s, K1:%s, K2:%s, mf_dim:%s, lambda_d:%s, lambda_t:%s, alpha:%s, beta:%s, theta:%s, max_iter:%s" % (self.cfix, self.K1, self.K2, self.num_factors, self.lambda_d, self.lambda_t, self.alpha, self.beta, self.theta, self.max_iter)
